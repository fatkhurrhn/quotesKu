...
export default function Storythur() {
  ...
  useEffect(() => {
    const savedMarkStatus = localStorage.getItem("quoteMarkStatusStorythur");
    if (savedMarkStatus) {
  ...
  const saveMarkStatus = (quoteId, status) => {
    ...
    localStorage.setItem("quoteMarkStatusStorythur", JSON.stringify(newStatus));
  };
  ...
  useEffect(() => {
    const fetchQuotes = async () => {
      try {
        ...
        const storythurQuotes = approved.filter(
          (quote) => quote.author?.toLowerCase() === "storythur"
        );

        setQuotes(storythurQuotes);
        setFilteredQuotes(storythurQuotes);
      } catch (err) {
  ...
  const getDisplayMessage = () => {
    ...
    return `${quotes.length} quotes dari Storythur`;
  };
    ...
    <div className="min-h-screen bg-[#f9fafb] pb-16">
      ...
      <div className="bg-gradient-to-r from-[#355485] to-[#4f90c6] pt-10 pb-8 rounded-b-3xl shadow-md">
        <div className="max-w-lg mx-auto px-5">
          <div className="text-center">
            ...
            <h1 className="text-2xl font-bold text-white mb-1">Storythur</h1>
            <p className="text-[#cbdde9] text-xs">Kumpulan quotes inspiratif untuk kehidupan</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-lg mx-auto px-5 -mt-5 mb-4">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Cari quotes dari Storythur..."
          ...
          <div className="space-y-3">
            {filteredQuotes.length > 0 ? (
                    ...
                    <div className="flex justify-between items-start mb-2 pb-2 border-b border-[#e5e7eb]">
                      <div className="flex items-center gap-2">
                        <div className="w-6 h-6 bg-[#cbdde9] rounded-lg flex items-center justify-center">
                          <i className="ri-book-line text-[#355485] text-xs"></i>
                        </div>
              ...
              <div className="text-center py-12 bg-white rounded-xl border border-[#e5e7eb]">
                ...
                <p className="text-[#6b7280] text-sm">Tidak ada quote dari Storythur</p>
                {searchTerm && (
                  <button
                    onClick={() => setSearchTerm("")}
